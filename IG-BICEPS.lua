...
local ions_info = false -- If true ions are terminated at plane and if talse first hit principle stands (from the moment the first ion hits the plane all others are killed, used to connect to the other part of the setup)
local ions_per_crystal = 10 -- amount of ions per crystal
...
local DC_BICEPS_1_trap = 47.55
local DC_BICEPS_2_trap = 17.55
local DC_BICEPS_3_trap = 47.55

-- DC potentials for BICEPS electrode segments (capturing)
local DC_BICEPS_1_capture = 47.55
local DC_BICEPS_2_capture = 17.55
local DC_BICEPS_3_capture = 47.55
...
local switch_time = 1000   -- µs (86.236181 is the ion time of birth in the simulation)

-- Laser and simulation parameters
local Irel = 100
local laser_offset = 1
local simulation_length = 500
...
function segment.flym()
    ...
    if not ions_info then
        local filename = "IG-BICEPS_trapping.fly2"
        do
    ...
    if record_trajectory then
        do
            ...
            file:write("x_positions(mm),y_positions(mm),z_positions(mm)\n")
            file:close()
        end
    end
...
function segment.fast_adjust()
    ...
    -- BICEPS electrodes
    adj_elect18 = DC_BICEPS_1 - RF_BICEPS
    adj_elect19 = RF_BICEPS
    adj_elect20 = DC_BICEPS_2 - RF_BICEPS
    adj_elect21 = RF_BICEPS
    adj_elect22 = DC_BICEPS_3 - RF_BICEPS
...
local test
if not ions_info then
    test_plane1 = TP(0, 0, IG_testplane, 0, 0, -1,
        function()
            if not first_hit then
                mark()
                print('First hit ion number: ' .. ion_number)
                first_hit = true
            end
        end
    )
...
segment.other_actions = function()
    ...
    -- Call the test planes' other_actions
    test_plane1.other_actions()
    test_plane2.other_actions()
    test_plane3.other_actions()

    if ion_time_of_flight > simulation_length then
	print(ion_pz_mm)
	ion_splat = 1
    end
   ...
   if first_hit then
	...
	if ions_transported1 % ions_per_crystal == 0 then
	    current_bunch = current_bunch + 1
	    print('Bunch splatted: ' .. current_bunch)
	    first_hit = false
	end
    end
...
function segment.terminate_run()
    if terminate == 0 then
	    ...
	    if not ions_info then
    	    local file = io.open("IG-BICEPS_trapping.fly2", "a")
    	    local ke1 = compute_kinetic_energy(vx1, vy1, vz1, mass)
    	    file:write("  coordinates = 0,\n")

    	    for i = 1, ions_transported1 do
        	file:write(string.format([[
        	    ...
        	    ]], tof1[i], mass, charge, ke1[i], vx1[i], vy1[i], vz1[i], x1[i], y1[i], z1[i]))
    	    end
    	    file:close()
	    end
